...
class MetaLayer_ml2(torch.nn.Module):
    def __init__(self, input_node_rep_dim, input_edge_rep_dim, input_subgraph_rep_dim, input_global_rep_dim,
                 output_node_rep_dim, output_edge_rep_dim, output_subgraph_rep_dim, output_global_rep_dim, hidden,
                 dropout):
        super(MetaLayer_ml2, self).__init__()
        self.edge_model = EdgeModel_ml2(2 * input_node_rep_dim + input_edge_rep_dim + input_global_rep_dim, hidden,
                                        output_edge_rep_dim, dropout)
        self.node_model = NodeModel_ml2(input_node_rep_dim + output_edge_rep_dim, hidden, hidden,
                                        hidden + input_node_rep_dim + input_global_rep_dim, hidden, output_node_rep_dim,
                                        dropout)
        self.subgraph_model = SubgraphModel_ml2(2 * input_subgraph_rep_dim + output_node_rep_dim + input_global_rep_dim,
                                                hidden, output_subgraph_rep_dim, dropout)
        self.global_model = GlobalModel_ml2(
            input_global_rep_dim + output_node_rep_dim + output_edge_rep_dim + output_subgraph_rep_dim, hidden,
            output_global_rep_dim, dropout)
...
class MLNet2(torch.nn.Module):
    def __init__(self, num_node_features, num_edge_features, num_global_features, hidden, dropout, num_tasks, depth,
                 graph_level_feature):
        ...
        if self.graph_level_feature:
            ...
            self.mlp2 = MLP(num_node_features + hidden, hidden, num_tasks, dropout, F.relu)
            self.mlp3 = MLP(num_edge_features + hidden, hidden, num_tasks, dropout, F.relu)
            self.mlp4 = MLP(hidden, hidden, num_tasks, dropout, F.relu)
        else:
            self.mlp_global = MLP(num_edge_features, hidden, hidden, dropout, F.relu)
        ...
        self.gn = torch.nn.ModuleList(
            [MetaLayer_ml2(hidden, hidden, hidden, hidden, hidden, hidden, hidden, hidden, hidden, dropout) for i in
             range(self.depth)])
    ...
    def forward(self, batch_data):
        ...
        if self.graph_level_feature:  ### Use rdkit_2d_normalized_features
            ...
            aux_feat = batch_data.x
            aue_feat = batch_data.edge_attr
        ...
        for i in range(self.depth):
            x, edge_attr, x_clique, u = self.gn[i](x, edge_index, edge_attr, u, tree_edge_index, atom2clique_index,
                                                   x_clique, ori_batch, tree_batch)
        ...
        if self.graph_level_feature:
            ...
            edge_attr = torch.cat([edge_attr, aue_feat], dim=1)
        out = self.mlp1(u)
        out_x = self.mlp2(x)
        out_edge = self.mlp3(edge_attr)
        out_x_clique = self.mlp4(x_clique)

        return out_x, out_edge, out_x_clique, out
...
class MetaLayer_ml3(torch.nn.Module):
    def __init__(self, input_node_rep_dim, input_edge_rep_dim, input_global_rep_dim, output_node_rep_dim,
                 output_edge_rep_dim, output_global_rep_dim, hidden, dropout):
        ...
        self.node_model = NodeModel_ml2(input_node_rep_dim + output_edge_rep_dim, hidden, hidden,
                                        hidden + input_node_rep_dim + input_global_rep_dim, hidden, output_node_rep_dim,
                                        dropout)
        self.global_model = GlobalModel_ml3(input_global_rep_dim + output_node_rep_dim + output_edge_rep_dim, hidden,
                                            output_global_rep_dim, dropout)
...
class MLNet3(torch.nn.Module):
    def __init__(self, num_node_features, num_edge_features, num_global_features, hidden, dropout, num_tasks, depth,
        ...
        self.gn = torch.nn.ModuleList(
            [MetaLayer_ml3(hidden, hidden, hidden, hidden, hidden, hidden, hidden, dropout) for i in range(self.depth)])
        self.norm_node = torch.nn.ModuleList([SizeNorm() for i in range(self.depth + 1)])
        self.norm_edge = torch.nn.ModuleList([SizeNorm() for i in range(self.depth + 1)])
        self.norm_subgraph = torch.nn.ModuleList([SizeNorm() for i in range(self.depth + 1)])
        self.bn_node = torch.nn.ModuleList([BatchNorm(hidden) for i in range(self.depth + 1)])
